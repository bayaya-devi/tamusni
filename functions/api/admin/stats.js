...
export async function onRequestGet(context) {
  ...
  const [users, subscribers, favorites, live, content, comments, forumPosts, reports, views] = await Promise.all([
    ...
    context.env.DB.prepare("SELECT COUNT(*) AS count FROM forum_posts WHERE status IN ('pending','reported')").first(),
  ...
  return json({ users: users?.count || 0, subscribers: subscribers?.count || 0, favorites: favorites?.count || 0, live: live?.count || 0, content: content?.count || 0, comments: Number(comments?.count || 0) + Number(forumPosts?.count || 0), reports: reports?.count || 0, views: views?.count || 0 });
}
