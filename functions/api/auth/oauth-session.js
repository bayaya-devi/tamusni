import { createSessionCookie, hashPassword, json, readBody, sameOrigin } from "../../_lib/auth.js";
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    let user = await context.env.DB.prepare("SELECT id,name,email,role,created_at FROM users WHERE email=? LIMIT 1").bind(email).first();
    if (!user) { user = { id: crypto.randomUUID(), name: profile.user_metadata?.full_name || profile.user_metadata?.name || email.split("@")[0], email, role: "USER", created_at: new Date().toISOString() }; await context.env.DB.prepare("INSERT INTO users (id,name,email,password_hash,role,created_at,email_verified_at) VALUES (?,?,?,?,?,?,?)").bind(user.id,user.name,email,await hashPassword(crypto.randomUUID()+crypto.randomUUID()),"USER",user.created_at,new Date().toISOString()).run(); }
