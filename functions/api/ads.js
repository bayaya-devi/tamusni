...
export async function onRequestGet(context) {
  ...
  const item = await context.env.DB.prepare("SELECT id,placement,headline,body,image_url,target_url,advertiser FROM advertisements WHERE placement=? AND status='active' AND (starts_at IS NULL OR starts_at<=?) AND (ends_at IS NULL OR ends_at>?) ORDER BY updated_at DESC LIMIT 1").bind(placement, now, now).first();
